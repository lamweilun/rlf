...
int main() {
    ...
    engine.setSetupFunc([]() {
        ...
        rlf::TypeManager::getInstance().registerType<ark::Node::PlayerNode>();
        rlf::TypeManager::getInstance().registerType<ark::Node::BallNode>();
        rlf::TypeManager::getInstance().registerType<ark::Node::BlockNode>();
    });
    ...
    engine.setShutdownFunc([](std::shared_ptr<rlf::Node::BaseNode>) {
        static constexpr std::string_view appName     = "arkanoid";
    ...
    // Run the engine
    rlf::Engine::Config config{1920, 1080, "arkanoid"};
    engine.run(config);
}
